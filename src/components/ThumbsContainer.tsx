import { createRef, FC, MouseEvent as ReactMouseEvent, RefObject, useEffect, useState } from 'react'
...
export const ThumbsContainer: FC<ThumbsContainerProps> = ({ images, onThumbClick }) => {
  ...
  const calculateMaxThumbWidth = (maxThumbs: number = CAROUSEL_OPTIONS.MAX_THUMBS) => {
    const singleGapPX = Number(theme.spacing(1).replace('px', ''))
    return thumbContainerWidth / maxThumbs - singleGapPX
  }
